        ...
        if (!Array.isArray(defPrefs[key])) {
          for (pref in defPrefs[key]) {
            if (
              ...
              newPrefs[key][pref] = (
                !prefs_ || prefs_[key][pref] === void 0 ? defPrefs : prefs_
              )[key][pref];
            }
          }
        }
  ...
  switch (msg.cmd) {
    ...
    case "resolve":
      ...
      xhr.onloadend = function () {
        ...
        if (
          /^(image|video|audio)\//i.test(this.getResponseHeader("Content-Type"))
        ) {
          data.m = msg.url;
          data.noloop = true;
          console.warn(
        ...
        if (!base_url) {
          ...
          if (
            (base_url = /<base\s+href\s*=\s*("[^"]+"|'[^']+')/.exec(base_url))
          ) {
          ...
          group = !group.test(sel)
            ...
            : sel.replace(group, function (m, prefix, id) {
                return id < data.params.length && prefix !== "\\"
                  ? prefix +
                      (data.params[id] ? RegExp.escape(data.params[id]) : "")
                  : m;
              });
  ...
  function () {
    cfg.get("version", function (items) {
      ...
      if (version.current !== app.version) {
        ...
        console.info(
          app.name + " has been " + (oldVersion ? "updated!" : "installed!")
        );
