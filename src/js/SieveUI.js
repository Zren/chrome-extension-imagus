...
var sieve_sec, sieve_container, SieveUI = {
	loaded: false,
	lastClicked: null,
	lastXY: [],
	cntr: 0,
	...
	init: function() {
		...
		sieve_sec.querySelector('.action_buttons').onclick = function(e) {
			switch (e.target.textContent) {
				case '●': SieveUI.select('add'); break;
				case '○': SieveUI.select('remove'); break;
				case '◐': SieveUI.select('toggle'); break;
				case '+': SieveUI.add(); break;
				case '-': SieveUI.remove(); break;
				case 'Ø': SieveUI.disable(); break;
				case '↓': ImprtHandler(_('NAV_SIEVE'), SieveUI.load); break;
				case '↑': SieveUI.exprt(e); break;
				case '⇓': SieveUI.update(); break;
				case '≡':
			...
			if (Object.keys(local_sieve).length) {
				...
				for (name in local_sieve) {
					...
					if (visible_rules[name]) {
						if (visible_rules[name].classList.contains('opened')) {
				...
				if (sfrag.childNodes.length) {
					if (sieve_container.firstElementChild) {
						sieve_container.insertBefore(sfrag, sieve_container.firstElementChild);
					} else {
	...
	prepareRules: function(ignore_dupes) {
		var
			i, j, params, param, rule, opt_name,
			rgxWhitespace = /\s+/g,
			output = {},
			dupes = [],
		...
		for (i = 0; i < rules.length; ++i) {
			...
			if (!opt_name && (!rule.nextElementSibling.textContent || [].some.call(
					rules[i].querySelectorAll('input[type="text"], textarea'),
					some_func
				))) {
		...
		if (!ignore_dupes && dupes.length) {
			...
			SieveUI.search_f.value = dupes.filter(function(el, idx, s) {
			...
			for (j = 0; j < params.length; ++j) {
				...
				switch (param) {
					...
					case 'note':
						...
						/* falls through */
					case 'link': case 'url': case 'res': case 'img': case 'to':
						if (params[j].value !== '') {
							rule[param] = params[j].value.replace(/\r\n?/g, '\n');
						}
					break;

					case 'link_ci': case 'img_ci':
					case 'link_dc': case 'img_dc':
					case 'link_loop': case 'img_loop':
	...
	genData: function(container, data) {
		++this.cntr;
		var vals = container.lastChild, c = '[' + this.cntr + ']',
				sd = data || this.sieve[container.rule] || {};

		buildNodes(vals, [
			{tag: 'label', nodes: [{tag: 'input', attrs: {type: 'checkbox', name: 'useimg'}}, {tag: 'label', attrs: {'class': 'checkbox'}}, ' ' + _('SIV_USEIMG')]}, ' ',
			{tag: 'input', attrs: {type: 'text', name: 'link', placeholder: 'link', 'class': 'sieve_shorter_inp'}},
			{tag: 'input', attrs: {type: 'checkbox', id: 'link_ci', name: 'link_ci'}}, {tag: 'label', attrs: {'class': 'checkbox'}},
			{tag: 'input', attrs: {type: 'checkbox', id: 'link_dc', name: 'link_dc'}}, {tag: 'label', attrs: {'class': 'checkbox'}},
			{tag: 'input', attrs: {type: 'checkbox', id: 'link_loop', name: 'link_loop'}}, {tag: 'label', attrs: {'class': 'checkbox'}},
			...
			{tag: 'input', attrs: {type: 'checkbox', id: 'img_ci', name: 'img_ci'}}, {tag: 'label', attrs: {'class': 'checkbox'}},
			{tag: 'input', attrs: {type: 'checkbox', id: 'img_dc', name: 'img_dc'}}, {tag: 'label', attrs: {'class': 'checkbox'}},
			{tag: 'input', attrs: {type: 'checkbox', id: 'img_loop', name: 'img_loop'}}, {tag: 'label', attrs: {'class': 'checkbox'}},
			...
			if (vals[i].name) {
				if (sd[vals[i].name]) {
					if (vals[i].type === 'checkbox') {
						vals[i].defaultChecked = vals[i].checked = !!sd[vals[i].name];
					} else {
						vals[i].defaultValue = vals[i].value = sd[vals[i].name] || '';
					}
				}
	...
	genEntry: function(name, data) {
		...
		buildNodes(container, [
			{tag: 'span'}, {tag: 'div', attrs: {'data-form': '1'}}
		]);
	...
	add: function() {
		...
		var rd = sieve_container.firstElementChild, rd_fc = rd.firstElementChild;
	...
	move: function(e) {
		...
		var div = e.target.parentNode, i, list;

		if (e.target.isContentEditable || e.target.nodeName !== 'SPAN'
			|| div.classList.contains('opened') || e.button !== 0 ) {
			return;
		}
		...
		document.onmousemove = function(e) {
			if ( Math.abs(SieveUI.lastXY[0] - e.clientX) < 4
				&& Math.abs(SieveUI.lastXY[1] - e.clientY) < 4 ) {
				return;
			}

			div.style.cssText = 'left:' + (e.clientX + 15) + 'px;'
				+ 'top:' + (e.clientY + 15) + 'px';
	...
	exprt: function(e) {
		...
		if ((exp = JSON.stringify(exp, null, e.shiftKey ? 2 : 0)) !== '{}') {
	...
	remove: function() {
		...
		var i = 0, list = sieve_container.querySelectorAll('div.selected');
	...
	search: function() {
		var
			what = RegExp(SieveUI.search_f.value.trim() || '.', 'i'),
			list = sieve_container.children,
			i = list.length;

		while (i--) {
			if (list[i].firstElementChild.textContent) {
				list[i].classList[what.test(list[i].firstElementChild.textContent) ? 'remove' : 'add']('hidden');
			}
		}
