(function(win, doc) {
    ...
    var imageSendTo = function(sf) {
        ...
        for (; i < hosts.length; ++i)
            if (sf.host === i || sf.host === void 0 && hosts[i][0][0] === "+") go.push(hosts[i][1].replace("%url", encodeURIComponent(sf.url)));
        Port.send({
            "cmd": "open",
            "url": go,
            "nf": !!sf.nf
    ...
    var PVI = {
        ...
        create: function() {
            ...
            PVI.DIV.style.cssText = "margin: 0; padding: 0; " + (cfg.hz.css || "") + "; visibility: visible; cursor: default; display: none; z-index: 2147483647; " + "position: fixed !important; box-sizing: content-box !important; left: auto; top: auto; right: auto; bottom: auto; width: auto; height: auto; max-width: none !important; max-height: none !important; ";
            PVI.DIV.curdeg = 0;
            PVI.LDR.wh = [35, 35];
            ...
            if (platform.chrome || platform.maxthon) PVI.VID.poster = "data:image/gif;base64,R0lGODlhAQABAAAAACwAAAAAAQABAAA=";
        ...
        createCAP: function() {
            ...
            buildNodes(PVI.CAP, [{
                tag: "b",
                attrs: {
                    style: "display: none; " + platform["transition_css"] + ": background-color .1s; border-radius: 3px; padding: 0 2px"
                }
            }, " ", {
                tag: "b",
                attrs: {
        ...
        onAttrChange: function(e) {
            ...
            switch (e.attrName) {
                case "style":
                    ...
                    if ((!bgImg || e.prevValue.indexOf(bgImg.slice(5, -2)) !== -1) && e.prevValue.indexOf("opacity") === -1 && target.style.cssText.indexOf("opacity") === -1) return;
                case "href":
                case "src":
                case "title":
                case "alt":
        ...
        getImages: function(el) {
            ...
            if (isHTMLElement)
                ...
                } else if (!el.childElementCount && el.parentNode.childElementCount <= 5 && (el.localName === "img" ? el.src.lastIndexOf("data:", 0) === 0 || el.naturalWidth < 3 || el.naturalHeight < 3 || el.style.opacity === "0" : !/\S/.test(el.textContent)) && el.style.backgroundImage[0] !== "u") {
            ...
            if (el.style.backgroundImage[0] === "u") imgs.imgBG_o = el.style.backgroundImage;
                ...
                if (p.offsetParent === el.offsetParent && p.style && p.style.backgroundImage[0] === "u")
                    if (Math.abs(p.offsetLeft - el.offsetLeft) <= 10 && Math.abs(p.offsetTop - el.offsetTop) <= 10 && Math.abs(p.clientWidth - el.clientWidth) <= 30 && Math.abs(p.clientHeight - el.clientHeight) <= 30) imgs.imgBG_o = p.style.backgroundImage
            }
        ...
        find: function(trg) {
            ...
            do {
                ...
                if (trg instanceof win.HTMLElement) {
                    ...
                    if (n.getAttributeNS) {
                        n = {
                            href: n.getAttributeNS("http://www.w3.org/1999/xlink", "href")
                        };
                        if (!n.href) break
                    }
        ...
        history: function(manual) {
            var url, i, n;
            if (!PVI.CNT || !PVI.TRG || platform.crx && chrome.extension.inIncognitoContext || platform.maxthon) return;
            if (manual) {
        ...
        wheeler: function(e) {
            ...
            if (PVI.fullZm && PVI.fullZm < 4) {
                if (d !== null) PVI.resize((e.deltaY || -e.wheelDelta) > 0 ? "-" : "+", PVI.fullZm > 1 ? e.target === PVI.CNT ? [e.offsetX || e.layerX || 0, e.offsetY || e.layerY || 0] : [] : null);
                pdsp(e);
                return
            }
            PVI.lastScrollTRG = PVI.TRG;
        ...
        m_over: function(e) {
            ...
            if (cache || src || (src = PVI.find(trg)) || src === null) {
                ...
                if (!cache && (!cfg.hz.waitHide || cfg.hz.delay < 15) && (PVI.fireHide && PVI.state > 2 || (PVI.state === 2 || PVI.hideTime && Date.now() - PVI.hideTime < 200))) {
