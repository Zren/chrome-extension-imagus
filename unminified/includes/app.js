    ...
    for (var i = 0, l = nodes.length; i < l; i++) {
        ...
        if (nodes[i].attrs)
            for (var attr in nodes[i].attrs)
                if (attr === "style") node.style.cssText = nodes[i].attrs[attr];
                else node.setAttribute(attr, nodes[i].attrs[attr]);
        if (nodes[i].nodes) buildNodes(node, nodes[i].nodes);
        else if (nodes[i].text) node.textContent = nodes[i].text;
        fragment.appendChild(node)
    }
    if (fragment.childNodes.length) host.appendChild(fragment);
    return host
}
...
window.addEventListener("keydown", function(e) {
    if (platform.onkeydown) platform.onkeydown(e)
}, true);
try {
...
Port = {
    listen: function(listener) {
        ...
        if (_) this.listener = function(message, sender) {
            if (!sender) listener(message)
        };
        else this.listener = listener;
        chrome.runtime.onMessage.addListener(this.listener)
    },
    send: function(message) {
        if (Port.listener) chrome.runtime.sendMessage(message, Port.listener);
    ...
    platform.insertHTML = function(node, str) {
        ...
        var cleanContainer = function(container) {
            ...
            while (i--) {
                ...
                while (j--)
                    if (!allowedAttrs.test(n.attributes[j].name)) n.removeAttribute(n.attributes[j].name);
                if (n.childElementCount) cleanContainer(n)
            }
        };
        platform.insertHTML = function(node, str) {
            ...
            while (safeContainer.firstChild) frag.appendChild(nodeDoc.adoptNode(safeContainer.firstChild));
