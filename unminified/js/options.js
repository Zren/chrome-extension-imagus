...
var processLNG = function(nodes) {
    ...
    while (i--) {
        ...
        while (l--) {
            ...
            } else platform.insertHTML(els[l], string);
...
var prefs = function(data, options, ev) {
    ...
    data = {};
    for (i = 0; i < 5; ++i)
        if (pref_keys[i] in cfg) data[pref_keys[i]] = cfg[pref_keys[i]];
...
window.onhashchange = function() {
    ...
    if (!section.lng_loaded)
        ...
        } else if (hash === "grants") section.querySelector(".action_buttons").onclick = function(e) {
        ...
        Port.listen(function(response) {
            ...
            var lng_map = function(el, idx) {
                ...
                td2.nodes.push(el.email || el.web ? {
                    tag: "a",
                    attrs: {
                        href: el.email ? "mailto:" + el.email : el.web
                    },
                    text: el.name
                } : el.name)
            };
...
window.addEventListener("load", function() {
    ...
    if (tmp = document.querySelectorAll('input[type="color"] + output + input[type="range"], textarea[name="tls_sendToHosts"]')) {
        ...
        [].forEach.call(tmp, function(el) {
            if (el.nodeName === "TEXTAREA") el.oninput = function() {
    ...
    document.forms[0].onchange = function(e) {
        ...
        if (!defval) return;
        if (t.type === "checkbox" && t[defval + "Checked"] !== t.checked || t.type !== "checkbox" && t[defval + "Value"] != t.value) input_changes[t.name] = true;
