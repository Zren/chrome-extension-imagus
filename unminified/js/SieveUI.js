var sieve_sec, sieve_container, SieveUI = {
    ...
    init: function() {
        ...
        sieve_container.onkeydown = function(e) {
            ...
            if (e.keyCode === 27 || e.keyCode === 13) {
                ...
                if (e.keyCode === 27 && rname === "" && t.nextElementSibling.textContent && [].every.call(t.parentNode.querySelectorAll('input[type="text"], textarea'), function(el) {
    ...
    prepareRules: function(ignore_dupes) {
        var i, j, params, param, rule, opt_name, rgxWhitespace = /\s+/g,
            output = {},
            dupes = [],
    ...
    click: function(e) {
        ...
        if (e.button === 0) {
            ...
            if (e.shiftKey && SieveUI.lastClicked !== null) {
                ...
                SieveUI.select(sieve_container.children[SieveUI.lastClicked].classList.contains("selected") ? "add" : "remove", Math.min(SieveUI.lastClicked, currentIndex), Math.max(SieveUI.lastClicked, currentIndex) + 1)
    ...
    move: function(e) {
        ...
        document.onmousemove = function(e) {
            ...
            for (i = 0; i < list.length; ++i)
                if (div !== list[i]) list[i].classList.add("move_multi")
        };
    ...
    exprt: function(e) {
        ...
        if (!sieve) return;
        if (list.length)
            for (i = 0; i < list.length; ++i) exp[list[i].rule] = sieve[list[i].rule];
        else exp = sieve;
    ...
    disable: function() {
        ...
        while (i--)
            if (!list || cn[i].classList.contains("selected")) cn[i].classList.toggle("disabled")
    },
    remove: function() {
        ...
        var i = 0,
            list = sieve_container.querySelectorAll("div.selected");
        if (list.length)
            for (; i < list.length; ++i) sieve_container.removeChild(list[i]);
